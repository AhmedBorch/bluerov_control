...
class MyPythonNode(Node):
    ...
    def AlfaBetaFilter(self,zvalue):
        ...
        dt = 1/58 #58 hz
        #init MOVE ALL THIS 
        self.z_filter = 0 #maybe dont need to be init
        self.w_filter = 0 #maybe also doesnt need to be init
        self.z_filter_1 = 0 #maybe init in the place where this is called for the first time
        self.w_filter_1 = 0 #also maybe init where is called for the 1st time
        self.pred_error = 0 #maybe doesnt need to be init
        self.z_sensor = zvalue #this is the sensor measurement 

        self.z_filter = self.z_filter_1 + (self.w_filter_1 * dt)
        self.w_filter = self.w_filter_1

        self.pred_error = self.z_sensor - self.z_filter

        self.z_filter += alfa * self.pred_error
        self.w_filter += (beta * self.pred_error)/dt
    ...
    def RelAltCallback(self, data):
        ...
        if (self.init_p0):
            # 1st execution, init
            self.depth_p0 = data.data #or be changed to the specific desired value put the value instead of data
            self.init_p0 = False
        
        # set servo depth control here
        kp = 1.0
        ki = 0.0
        error = self.depth_p0 - data.data
